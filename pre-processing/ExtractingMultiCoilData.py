...
from utils.prostate_rec.utils import t2_reconstruction
...
for file_name in h5_files:
    ...
    with h5py.File(file_path, 'r') as h5_file:
        ...
        calibration_data = h5_file['calibration_data']
        hdr = h5_file['ismrmrd_header']
        hdr = hdr[()].decode('utf-8')
        kspace = np.array(kspace)
        kspace= t2_reconstruction(kspace_data=kspace, calib_data=calibration_data, hdr=hdr)
        [average, slice_number, coil_number, readout, phase]=np.shape(kspace)
        imgs = ifft2(kspace)
