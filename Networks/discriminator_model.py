...
class PatchGAN(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=6, norm_layer=nn.BatchNorm2d,crop_center = None,FC_bottleneck=False):
        ...
        sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True),
                    nn.Conv2d(ndf, ndf, kernel_size=kw, stride=2, padding=padw), norm_layer(ndf * nf_mult), nn.LeakyReLU(0.2, True)]
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
            nf_mult = min(2 ** n, 2 ** n_layers)
            sequence += [
        ...
        nf_mult_prev = nf_mult
        nf_mult = min(2 ** n_layers, 2 ** n_layers)
        sequence += [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ...
        if FC_bottleneck:
            sequence +=[nn.AdaptiveAvgPool2d((1,1)),
                        nn.Flatten(),nn.Linear(ndf * nf_mult,64),
                        nn.LeakyReLU(0.2, True),
                        nn.Linear(64,1)]
        else:
            # sequence += [
            ...
            sequence += [nn.AdaptiveAvgPool2d((1,1)),
                        nn.Flatten(),
                        nn.Linear(ndf * nf_mult, ndf * (2 ** (n_layers-2))), nn.LeakyReLU(0.2, True), nn.Dropout(),
                        nn.Linear(ndf * (2 ** (n_layers-2)), 1)]  # output 1 channel prediction map #nn.Sigmoid() abandoned
